...
def test_build_profiles_progress_callback():
    # Callback de progression (BuildProfilesWorker) : appele a chaque element
    # traite (petit total ici), done croissant jusqu'a total.
    w = VTKViewerWidget.__new__(VTKViewerWidget)
    w._color_by_section = False
    sec = _section()
    lines = [[(0, i, 0), (4, i, 0)] for i in range(5)]
    secs = [sec] * 5

    calls = []
    w._build_profiles_polydata(lines, secs, element_indexes=list(range(5)), progress_cb=lambda done, total: calls.append((done, total)))
