...
import flight_navigation as flight


def test_flight_direction_vectors_yaw_zero_points_along_y():
    forward, right, up = flight.direction_vectors(0.0)
...
def test_flight_direction_vectors_are_orthogonal_and_horizontal():
    forward, right, up = flight.direction_vectors(math.radians(37.0))
...
def test_flight_look_direction_matches_azimuth_elevation_convention():
    # Meme convention que scene_light.azimuth_elevation_to_position :
    # x = cos(pitch) * sin(yaw), y = cos(pitch) * cos(yaw), z = sin(pitch).
    dx, dy, dz = flight.look_direction(0.0, 0.0)
    assert (round(dx, 9), round(dy, 9), round(dz, 9)) == (0.0, 1.0, 0.0)
    dx, dy, dz = flight.look_direction(0.0, math.radians(90.0))
...
def test_flight_movement_vector_no_keys_is_zero():
    ...
    result = flight.movement_vector(set(), forward, right, up, bindings)
...
def test_flight_movement_vector_single_key_forward():
    forward, right, up = flight.direction_vectors(0.0)
    bindings = {"forward": "z", "backward": "s", "left": "q", "right": "d", "up": "a", "down": "e"}
    result = flight.movement_vector({"z"}, forward, right, up, bindings)
...
def test_flight_movement_vector_diagonal_is_normalized():
    forward, right, up = flight.direction_vectors(0.0)
    bindings = {"forward": "z", "backward": "s", "left": "q", "right": "d", "up": "a", "down": "e"}
    result = flight.movement_vector({"z", "d"}, forward, right, up, bindings)
...
def test_flight_movement_vector_opposite_keys_cancel():
    forward, right, up = flight.direction_vectors(0.0)
    bindings = {"forward": "z", "backward": "s", "left": "q", "right": "d", "up": "a", "down": "e"}
    result = flight.movement_vector({"z", "s"}, forward, right, up, bindings)
    assert result == (0.0, 0.0, 0.0)


def test_flight_safe_position_returns_unchanged_when_bounds_none():
    pos = (5.0, 5.0, 5.0)
    result = flight.safe_position(pos, None, (0.0, 1.0, 0.0))
    assert result == pos
...
def test_flight_safe_position_leaves_outside_position_unchanged():
    ...
    result = flight.safe_position(pos, bounds, (0.0, 1.0, 0.0))
    assert result == pos
...
def test_flight_controls_overlay_text_contains_bound_keys():
    ...
    cfg.set_language("fr")
    bindings = {"forward": "z", "backward": "s", "left": "q", "right": "d", "up": "a", "down": "e"}
    text = flight.controls_overlay_text(bindings)
    assert "Z/S" in text
    assert "Q/D" in text
    assert "A/E" in text
...
def test_flight_clamp_pitch_within_range_is_unchanged():
    ...
    assert flight.clamp_pitch(0.1, limit) == 0.1


def test_flight_clamp_pitch_clamps_above_limit():
    limit = math.radians(89.0)
    result = flight.clamp_pitch(math.radians(120.0), limit)
